...
var require_metadata = __commonJS({
  "src/shared/metadata.js"(exports, module2) {
    ...
    function resolveTmdbId(inputId, type) {
      ...
      return fetch(`${DOMAINS4.TMDB_API}/find/${encodeURIComponent(value)}?api_key=${TMDB_KEY}&external_source=imdb_id`).then((response) => response.ok ? response.json() : null).then((data) => {
        var _a;
    ...
    function getMetadata2(tmdbId, mediaType) {
      ...
      if (!cache.has(key)) {
        cache.set(key, resolveTmdbId(tmdbId, type).then((resolvedId) => resolvedId ? fetch(`${DOMAINS4.TMDB_API}/${type}/${resolvedId}?api_key=${TMDB_KEY}&append_to_response=external_ids`) : null).then((response) => (response == null ? void 0 : response.ok) ? response.json() : null).then((data) => {
          var _a;
          return data ? {
