/** Netcinez - generated from src/providers/netcinez.js */
...
var HEADERS = withReferer({ "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36" }, `${BASE_URL}/`);
function finalStreamsFromPage(pageUrl, pageReferer) {
  return __async(this, null, function* () {
    const pageDoc = parseHtml(yield getText(pageUrl, { headers: withReferer(HEADERS, pageReferer) }));
    ...
    if (!iframeUrl)
      return [];
    const iframeDoc = parseHtml(yield getText(iframeUrl, { headers: withReferer(HEADERS, pageUrl) }));
    ...
    for (const button of buttons.slice(0, 8)) {
      ...
      const finalUrl = yield resolveFinalUrl(intermediate, { headers: requestHeaders }).catch(() => null);
      if (finalUrl)
        streams.push(__spreadValues({ url: finalUrl, title: `Netcinez [${label || "Stream"}]`, label, headers: requestHeaders, subtitles: [] }, parseMediaAttributes(label, finalUrl)));
    }
...
function getStreams(tmdbId, mediaType, season = 1, episode = 1) {
  return __async(this, null, function* () {
    try {
      ...
      const searchDoc = parseHtml(yield getText(`${BASE_URL}/?s=${encodeURIComponent(title)}`, { headers: HEADERS }));
      ...
      if (!match)
        return [];
      if (mediaType !== "tv")
        return yield finalStreamsFromPage(match.url, BASE_URL);
      const showDoc = parseHtml(yield getText(match.url, { headers: HEADERS }));
      ...
      return episodeUrl ? yield finalStreamsFromPage(episodeUrl, match.url) : [];
    } catch (error) {
