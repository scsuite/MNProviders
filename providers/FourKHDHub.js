...
var FALLBACK_URL = "https://4khdhub.one";
var USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36";
function getBaseUrl() {
  return __async(this, null, function* () {
    try {
      return (yield getJson(DOMAINS_URL, { retries: 0 }))["4khdhub"] || FALLBACK_URL;
    } catch (_) {
      return FALLBACK_URL;
    }
  });
}
function resolveDownload(url, referer, label) {
  return __async(this, null, function* () {
    const headers = withReferer({ "User-Agent": USER_AGENT }, referer);
    const firstDoc = parseHtml(yield getText(url, { headers }).catch(() => ""));
    const next = absoluteUrl(firstDoc("#download").attr("href"), url);
    const candidates = next ? [next] : [url];
    if (next) {
      const nextDoc = parseHtml(yield getText(next, { headers: withReferer(headers, url) }).catch(() => ""));
      nextDoc("a.btn, a[href]").each((_, item) => {
        const href = absoluteUrl(nextDoc(item).attr("href"), next);
        if (href && candidates.length < 3)
          candidates.push(href);
      });
    }
    for (const candidate of candidates.slice(0, 3)) {
      const finalUrl = yield resolveFinalUrl(candidate, { headers: withReferer(headers, next || url) }).catch(() => null);
      if (finalUrl)
        return __spreadValues({ url: finalUrl, headers: withReferer(headers, next || url) }, parseMediaAttributes(label, finalUrl));
    }
    return null;
  });
}
function getStreams(tmdbId, mediaType, season = 1, episode = 1) {
  return __async(this, null, function* () {
    try {
      ...
      if (!match)
        return [];
      const pageDoc = parseHtml(yield getText(match.url, { headers }));
      const links = [];
      if (mediaType === "tv") {
        pageDoc("div.episode-download-item").each((_, item) => {
          ...
          const matches = marker && (marker.length === 3 ? Number(marker[1]) === Number(season) && Number(marker[2]) === Number(episode) : Number(marker[1]) === Number(episode));
          if (matches)
            pageDoc(item).find("a").each((__, anchor) => links.push({ url: absoluteUrl(pageDoc(anchor).attr("href"), match.url), label }));
        });
      } else {
      ...
      for (const link of links.filter((item) => item.url).slice(0, 3)) {
        const resolved = yield resolveDownload(link.url, match.url, link.label);
        if (resolved)
          streams.push(__spreadProps(__spreadValues({}, resolved), { title: `4KHDHub [${link.label || "Stream"}]`, label: link.label, subtitles: [] }));
      }
