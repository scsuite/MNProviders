/** Desicinemas - generated from src/providers/desicinemas.js */
...
var proxyUrl = (url) => `${PROXY}?url=${encodeURIComponent(url)}`;
function getStreams(tmdbId, mediaType, season = 1, episode = 1) {
  return __async(this, null, function* () {
    try {
      const { title } = yield getMediaInfo(tmdbId, mediaType);
      const searchDoc = parseHtml(yield getText(proxyUrl(`${BASE_URL}/?s=${encodeURIComponent(title)}`), { headers: HEADERS }));
      ...
      if (!match)
        return [];
      const pageDoc = parseHtml(yield getText(proxyUrl(match.url), { headers: HEADERS }));
      ...
      for (const box of boxes) {
        ...
        if (mediaType === "tv") {
          ...
          if (marker && (Number(marker[1]) !== Number(season) || Number(marker[2]) !== Number(episode)))
            continue;
        }
        const pageUrl = absoluteUrl(pageDoc(box).find("a").attr("href"), match.url);
        if (!pageUrl)
          continue;
        const embedDoc = parseHtml(yield getText(pageUrl, { headers: withReferer(HEADERS, match.url) }).catch(() => ""));
        ...
        if (!finalUrl)
          continue;
        streams.push(__spreadValues({ url: finalUrl, title: `Desicinemas [${label || "Stream"}]`, label, headers: requestHeaders, subtitles: [] }, parseMediaAttributes(label, finalUrl)));
      }
