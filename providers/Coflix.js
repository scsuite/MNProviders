/** Coflix - generated from src/providers/coflix.js */
...
var API_URL = `${BASE_URL}/wp-json/apiflix/v1`;
...
function getStreams(tmdbId, mediaType, season = 1, episode = 1) {
  return __async(this, null, function* () {
    try {
      const { title } = yield getMediaInfo(tmdbId, mediaType, { headers: HEADERS });
      if (!title)
        return [];
      const search = yield getJson(`${BASE_URL}/suggest.php?query=${encodeURIComponent(title)}`, { headers: HEADERS });
      const results = Array.isArray(search) ? search : [];
      const match = bestTitleMatch(results, title) || results[0];
      const pageUrl = absoluteUrl(match == null ? void 0 : match.url, BASE_URL);
      if (!pageUrl)
        return [];
      const pageHtml = yield getText(pageUrl, { headers: HEADERS });
      ...
      if (mediaType === "tv") {
        ...
        if (!postId)
          return [];
        const data = yield getJson(`${API_URL}/series/${postId}/${Number(season) || 1}`, { headers: HEADERS });
        const target = (data.episodes || []).find((item) => Number(item.number) === (Number(episode) || 1));
        streamPageUrl = absoluteUrl(target == null ? void 0 : target.links, pageUrl);
        if (!streamPageUrl)
          return [];
      }
      const streamHtml = yield getText(streamPageUrl, { headers: withReferer(HEADERS, pageUrl) });
      ...
      if (!iframeUrl)
        return [];
      const iframeHtml = yield getText(iframeUrl, { headers: withReferer(HEADERS, streamPageUrl) });
      const iframeDoc = parseHtml(iframeHtml);
      const candidates = [];
      iframeDoc("li[onclick]").each((_, item) => {
        var _a;
        const encoded = (_a = (iframeDoc(item).attr("onclick") || "").match(/showVideo\(['"]([^'"]+)/)) == null ? void 0 : _a[1];
        try {
          const url = absoluteUrl(decodeBase64(encoded), iframeUrl);
          if (url)
            candidates.push({ url, label: iframeDoc(item).text().trim() });
        } catch (_2) {
        }
      });
      const streams = [];
      for (const candidate of candidates) {
        ...
        if (finalUrl)
          streams.push({ url: finalUrl, quality: parseQuality(finalUrl, candidate.label), title: `Coflix [${candidate.label || "Stream"}]`, headers: requestHeaders, subtitles: [] });
      }
